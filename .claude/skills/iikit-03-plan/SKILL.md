...
1. Run setup script (choose based on platform):
   ...
   **Windows (PowerShell):**
   ```powershell
   pwsh .claude/skills/iikit-core/scripts/powershell/setup-plan.ps1 -Json
   ```
...
4. **Agent context update** (choose based on platform):
   ...
   **Windows (PowerShell):**
   ```powershell
   pwsh .claude/skills/iikit-core/scripts/powershell/update-agent-context.ps1 -AgentType claude
   ```

   This updates CLAUDE.md with the new technology stack using the [agent-file-template.md](.claude/skills/iikit-core/templates/agent-file-template.md).
