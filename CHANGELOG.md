...
### Features
- **`pre-commit.d/` extension point** (#61): IIKit's pre-commit hook now executes every executable in `.git/hooks/pre-commit.d/` on every IIKit success or no-op path — assertion check pass, nothing relevant staged (fast-path), and the degraded "IIKit scripts not found" warning path. Extensions are skipped only when IIKit explicitly blocks the commit, so they never see a partial pass. `/iikit-core init` provisions the directory with a discoverable `README` (marked `IIKIT-PRE-COMMIT-D`) explaining the convention. `/iikit-core uninit` removes that README and drops the dir when empty, reporting any user-authored scripts as preserved content. Lets downstream projects layer prettier/eslint/secret-scan checks without touching `.git/hooks/pre-commit` (which IIKit still owns) — the previous workaround relied on hook managers renaming the IIKit hook to `pre-commit.old`, which the assertion-integrity rule forbids. Assertion-integrity rule now points at the new extension point and explicitly bars hook managers that overwrite/rename `.git/hooks/pre-commit`. Reference doc at `.claude/skills/iikit-core/references/hook-chaining.md` with examples. BATS + Pester coverage.
