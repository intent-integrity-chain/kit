...
### Bug Fixes
- **Dev-time symlink replaced with `tessl install file:` workflow** (#82): The tracked symlink at `.tessl/plugins/tessl-labs/intent-integrity-kit` → `../../../tiles/intent-integrity-kit` is removed. Local development now uses Tessl's documented `tessl install file:./tiles/intent-integrity-kit [--watch-local]` workflow ([docs](https://docs.tessl.io/create/developing-plugins-locally)), which populates `.tessl/plugins/tessl-labs/intent-integrity-kit/` from the local source tree at install time. The symlink had three problems: it looked vendored (tripping `dependency-management` policy reviewers), it broke on the Tessl 0.81 `.tessl/tiles/` → `.tessl/plugins/` rename (fixed in #80), and it bypassed Tessl's own dependency tracking. New `CONTRIBUTING.md` documents the dev-setup loop. `.gitignore` allowlist exception lines removed; broad `.tessl/plugins/*` rule keeps the install output out of git. Adjacent fix: `.gitignore` patterns for Tessl-installed agent-skill symlinks corrected from `tessl:*` to also cover `tessl__*` (the actual current convention) across all four agent directories (`.claude/`, `.codex/`, `.gemini/`, `.opencode/`).
