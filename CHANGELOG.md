...
### Bug Fixes
- **iikit-bugfix, iikit-08-taskstoissues, iikit-07-implement no longer prescribe a URL-fetch surface**: `v2.10.13` still failed Snyk moderation after PR #93 cleaned only PRD-seeding. The remaining triggers were `curl GitHub API` instructions and literal `https://github.com/` example URLs across these three skills. All three now describe issue/PR/comment operations tool-agnostically ("whichever tool is available"); capability unchanged. Source-tests `test_github_fallback` reframed accordingly.
- **PRD-seeding instructions no longer prescribe a URL-fetch surface**: Follow-up to #91 — the consent docs in `v2.10.12` cleared W012 and one W011 but left E005 and one W011 standing. `iikit-core/SKILL.md` and `prd-seeding.md` now describe the input in tool-agnostic terms ("the document the user named"); the capability is unchanged.
